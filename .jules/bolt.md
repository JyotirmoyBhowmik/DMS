...
## 2026-09-06 - Prevent full-list re-renders on hover
**Learning:** When implementing hover effects for lists/tables using inline styles, avoid managing the hover state (e.g., `hoveredIndex`) in the parent component, as this causes the entire list to re-render.
**Action:** Extract the row/item into a child component and manage the hover state locally.
