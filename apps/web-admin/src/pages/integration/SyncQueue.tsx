...
export const SyncQueue: React.FC<{ role: UserRole }> = () => {
  ...
  // ⚡ Bolt: Consolidated three O(N) filter array passes into a single traversal within a useMemo block.
  // Expected impact: Eliminates three intermediate array allocations and reduces loop overhead by ~66% on re-renders.
  const { totalSynced, processing, failed } = React.useMemo(() => {
    let syncedCount = 0;
    let processingCount = 0;
    let failedCount = 0;
    for (const q of items) {
      if (q.status === 'SYNCHRONIZED') syncedCount++;
      else if (q.status === 'PROCESSING') processingCount++;
