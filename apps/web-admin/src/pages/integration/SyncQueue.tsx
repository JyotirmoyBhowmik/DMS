...
export const SyncQueue: React.FC<{ role: UserRole }> = () => {
  ...
  // ⚡ Bolt: Combined three independent O(N) .filter() passes into a single O(N) loop
  // inside useMemo. Expected impact: Minimizes iteration overhead and prevents
  ...
  const { totalSynced, processing, failed } = React.useMemo(() => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (status === 'SYNCHRONIZED') synced++;
      else if (status === 'PROCESSING') proc++;
      else if (status === 'FAILED') fail++;
    }
    return { totalSynced: synced, processing: proc, failed: fail };
  }, [items]);
