...
import { SEED_VAN_SALES } from '../../data/seed';
import { StatusBadge } from '../../components/StatusBadge';

export const VanSales: React.FC<{ role: UserRole }> = ({ role }) => {
  const totalDispatched = SEED_VAN_SALES.filter((v: any) => v.status === 'DISPATCHED').length;
  const totalDelivered = SEED_VAN_SALES.filter((v: any) => v.status === 'DELIVERED').length;
  const totalLoading = SEED_VAN_SALES.filter((v: any) => v.status === 'LOADING').length;
    ...
    <div style={{ padding: '24px', backgroundColor: '#F8FAFC', minHeight: '100vh', color: '#334155' }}>
      ...
      <div style={{ backgroundColor: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: '8px', overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {SEED_VAN_SALES.map((v: any) => (
              <tr key={v.id} style={{ borderBottom: '1px solid #E2E8F0' }}>
                <td style={{ padding: '12px 16px', fontWeight: '500' }}>{v.vanId}</td>
                <td style={{ padding: '12px 16px' }}>${v.orderValue}</td>
