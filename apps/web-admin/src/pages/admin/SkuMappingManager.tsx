...
export const SkuMappingManager: React.FC<{ role: UserRole }> = ({ role }) => {
  ...
  const filteredDistributors = useMemo(() => {
    ...
    return distributors.filter(
      (d) => d.name.toLowerCase().includes(searchLower) || d.id.toLowerCase().includes(searchLower),
    );
  ...
  const handleBulkMapCategory = async () => {
    ...
    setSavingMap(true);
    try {
      // ⚡ Bolt: Batch sequential API calls into concurrent requests
      // Expected impact: Significantly reduces total execution time when bulk mapping multiple SKUs by utilizing Promise.all.
      await Promise.all(
        availableCatalogSkus.map((item) =>
          fetch(`/api/v1/distributors/${selectedDistributorId}/skus`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
      ...
      setBanner({
        text: `Bulk mapped ${availableCatalogSkus.length} SKUs in category '${selectedCategory}'`,
        type: 'success',
      });
      fetchMappedSkus(selectedDistributorId);
    } catch (_e) {
  ...
  const handleBulkCopyMapping = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setBanner({
          text: json.message || 'Bulk copied SKU mapping successfully',
          type: 'success',
        });
        fetchMappedSkus(selectedDistributorId);
      } else {
    ...
    setMappedSkus((prev) =>
      prev.map((m) =>
        m.sku === skuId
          ? {
              ...m,
              overridePrice: isNaN(num) ? null : num,
              price: isNaN(num) ? m.masterPrice || m.price : num,
            }
          : m,
      ),
    );
  ...
  const handleMinQtyChange = (skuId: string, val: string) => {
    ...
    setMappedSkus((prev) =>
      prev.map((m) => (m.sku === skuId ? { ...m, minOrderQty: isNaN(num) ? 1 : num } : m)),
    );
  };
  ...
  if (!hasPermission) {
    return (
      ...
      >
        ...
        <p>
          Your current user role ({role}) does not have permission to manage distributor SKU catalog
          mappings.
        </p>
      </div>
    );
    ...
    >
      ...
      >
        <div>
          ...
          >
            Distributor SKU Catalog Mapping Manager
          </h1>
          <p style={{ color: tokens.colors.textMuted, margin: '4px 0 0', fontSize: '14px' }}>
            Map master tenant SKUs to specific distributors with custom price overrides and minimum
            order quantities
          </p>
        </div>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          >
            ...
            {distributors
              .filter((d) => d.id !== selectedDistributorId)
              .map((d) => (
                <option key={d.id} value={d.id}>
                  {d.name}
                </option>
            ...
            {availableCatalogSkus.length === 0 ? (
              ...
              availableCatalogSkus.map((item) => (
                ...
                >
                  <div>
                    <div
                      style={{ fontWeight: 700, fontSize: '13px', color: tokens.colors.textMain }}
                    >
                      {item.name}
                    </div>
                    <div style={{ fontSize: '11px', color: tokens.colors.textMuted }}>
                      SKU: {item.sku} • Category: {item.category} • Master Price: $
          ...
          <div style={{ maxHeight: '550px', overflowY: 'auto' }}>
            {loadingMapped ? (
              <div style={{ padding: '32px', textAlign: 'center', color: tokens.colors.textMuted }}>
                Loading mapped distributor catalog...
              </div>
              ...
              filteredMappedSkus.map((m) => (
                ...
                >
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: '11px', color: tokens.colors.textMuted }}>
                      SKU: {m.sku} • Master: ${m.masterPrice || m.price}
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <div style={{ textAlign: 'right' }}>
                      ...
                      <input
                        ...
                        step="0.01"
                        value={
                          m.overridePrice !== undefined && m.overridePrice !== null
                            ? m.overridePrice
                            : m.price
                        }
