...
export const EnterpriseHierarchy: React.FC<{ role: UserRole }> = ({ role }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [distJson, agentJson] = await Promise.all([distRes.json(), agentRes.json()]);
  ...
  const filteredDistributors = useMemo(() => {
    ...
    return distributors.filter((d) => {
      const matchesSearch =
        d.name.toLowerCase().includes(lowerSearchQuery) ||
        d.id.toLowerCase().includes(lowerSearchQuery) ||
        d.level.toLowerCase().includes(lowerSearchQuery);
  ...
  // Calculate total credit limit once
  const totalCreditLimit = useMemo(() => {
    return outlets.reduce((acc, o) => acc + (o.creditLimit || 0), 0);
  }, [outlets]);

  // Get Agent Recent Activity
  const getAgentActivity = (agent: SalesAgentNode) => {
    const agentOrders: SalesOrder[] = salesOrders.filter(
      (o) => o.agent === agent.name || o.agent.toLowerCase().includes('agent'),
    );
    const agentVisits: FieldVisit[] = fieldVisits.filter(
      (v) => v.agent === agent.name || v.agent.toLowerCase().includes('agent'),
    );
    ...
    >
      ...
      >
        <div>
          ...
          >
            Enterprise Scope & Hierarchy Tree
          </h1>
          <p style={{ color: tokens.colors.textMuted, margin: '4px 0 0', fontSize: '14px' }}>
            Single unified scope view: Tenant → Distributors → Sales Agents with live activity feeds
            & level metrics
          </p>
        </div>
        ...
        {isSuperAdmin && (
          ...
          >
            <span style={{ fontSize: '13px', fontWeight: 600, color: tokens.colors.textMuted }}>
              Tenant Scope:
            </span>
            <select
      ...
      {loading ? (
        <div style={{ padding: '32px', textAlign: 'center', color: tokens.colors.textMuted }}>
          Loading enterprise scope hierarchy...
        </div>
          ...
          {paginatedDistributors.map((dist, idx) => {
            ...
            return (
              <div
                ...
                style={{
                  borderBottom:
                    idx === paginatedDistributors.length - 1
                      ? 'none'
              ...
              >
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                    ...
                    <div>
                      <div
                        style={{ fontWeight: 700, color: tokens.colors.textMain, fontSize: '15px' }}
                      >
                        {dist.name}
                      </div>
                      <div style={{ fontSize: '12px', color: tokens.colors.textMuted }}>
                        ID: {dist.id} • Level: {dist.level}
                      </div>
                    </div>
                  </div>
                ...
                {isExpanded && (
                  ...
                  >
                    ...
                    >
                      <div>
                        <strong>Assigned Beat Routes:</strong> {beatRoutes.length} Active Routes
                      </div>
                      <div>
                        <strong>Total Stores & Credit Limits:</strong> $
                        {totalCreditLimit.toLocaleString()} limit
                      </div>
                    </div>
                  </div>
                )}
          ...
          >
            ...
            >
              <div>
                ...
                <p style={{ fontSize: '12px', color: tokens.colors.textMuted, margin: '2px 0 0' }}>
                  Sales Rep ID: {selectedAgent.id}
                </p>
              </div>
              <button
              ...
              return (
                <div>
                  ...
                  {activity.recentOrders.length === 0 ? (
                    ...
                    <div style={{ marginBottom: '16px' }}>
                      {activity.recentOrders.map((o: SalesOrder, idx: number) => (
                        ...
                        >
                          <div style={{ fontWeight: 700, color: tokens.colors.textMain }}>
                            Order #{o.id || idx + 1} - ${o.totalAmount || '120.00'}
                          </div>
                          <div style={{ color: tokens.colors.textMuted }}>
                            Status: {o.status || 'APPROVED'} • Store: {o.outlet || 'Retail Store'}
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  {activity.recentVisits.length === 0 ? (
                    ...
                    >
                      No recent GPS field visits recorded.
                    </div>
                  ) : (
                    <div>
                      {activity.recentVisits.map((v: FieldVisit, idx: number) => (
                        ...
                        >
                          <div style={{ fontWeight: 700, color: tokens.colors.textMain }}>
                            📍 Visit - {v.outlet || 'Outlet Visit'}
                          </div>
                          <div style={{ color: tokens.colors.textMuted }}>
                            Status: {v.status} • Time: {v.time}
                          </div>
                        </div>
                      ))}
                    </div>
