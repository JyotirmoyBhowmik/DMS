...
export const StockLedger: React.FC<{ role: UserRole }> = ({ role }) => {
  ...
  // Combined 4 separate unmemoized loops (O(4n) time complexity) into a single O(n) pass
  // wrapped in useMemo to prevent unnecessary recalculations on every render.
  // Impact: O(n) instead of O(4n), preventing layout thrashing on large inventories.
    ...
    >
      ...
      >
        {[
          ...
          >
            <div style={{ color: tokens.colors.textMuted, fontSize: '14px', marginBottom: '8px' }}>
              {stat.label}
            </div>
      ...
      {inventory.length === 0 ? (
        ...
        >
          <p style={{ color: tokens.colors.textMuted, fontSize: '14px', margin: 0 }}>
            No stock ledger items found in database catalog.
          </p>
        </div>
      ) : (
        <div
                      ...
                      <tr key={item.sku}>
                        ...
                        <td
                          style={{
                            ...
                            borderBottom: `1px solid ${tokens.colors.border}`,
                            fontWeight: 700,
                            color: isLow ? tokens.colors.danger : tokens.colors.textMain,
                          }}
                        >
                          {item.stock}
                        </td>
