...
import type { QueryResult, IConnectionClient } from '../postgres/client.js';
...
export class MigrationRunner {
   ...
   * with a warning.
   */
  async rollback(steps = 1): Promise<{ rolledBack: string[]; warnings: string[] }> {
    await this.ensureTrackingTable();
    ...
    for (const migration of toRollback) {
      ...
      if (!existsSync(downPath)) {
        warnings.push(
          `No rollback file found for ${migration.filename} (expected ${downFile})`,
        );
        continue;
      }
      ...
      await this.db.transaction(async (conn) => {
        await conn.query(sql);
        await conn.query(
          `DELETE FROM ${this.fqTable} WHERE version = $1`,
          [migration.version],
        );
      });
  ...
  async status(): Promise<MigrationStatus> {
    ...
    }>(`SELECT version, description, filename, checksum, applied_at, execution_time_ms
        FROM ${this.fqTable}
        ORDER BY version ASC`);

    const applied: AppliedMigration[] = result.rows.map((r) => ({
      version: r.version,
   ...
   */
  async validate(): Promise<MigrationValidation> {
    const errors: string[] = [];
    const warnings: string[] = [];
    ...
    for (let i = 1; i < versionNumbers.length; i++) {
      ...
      if (curr - prev > 1) {
        warnings.push(
          `Gap in version numbering between V${String(prev).padStart(3, '0')} and V${String(curr).padStart(3, '0')}`,
        );
      }
    }
    ...
    try {
      ...
      for (const appliedMig of applied) {
        ...
        if (!diskMig) {
          errors.push(
            `Applied migration ${appliedMig.version} (${appliedMig.filename}) not found on disk`,
          );
          continue;
        }
        const content = readFileSync(diskMig.filepath, 'utf-8');
        const diskChecksum = computeChecksum(content);
        if (diskChecksum !== appliedMig.checksum) {
          errors.push(
            `Checksum mismatch for ${appliedMig.version}: applied=${appliedMig.checksum}, disk=${diskChecksum}. ` +
   ...
   */
  private discoverMigrations(): MigrationInfo[] {
    ...
    return readdirSync(this.migrationsDir)
      ...
      .map((filename) => {
        ...
        return {
          ...
          filename,
          filepath: join(this.migrationsDir, filename),
        };
      })
