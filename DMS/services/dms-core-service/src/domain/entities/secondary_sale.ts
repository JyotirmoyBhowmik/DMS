/**
 * SecondarySale Domain Entity.
 * Represents distributor-to-retailer secondary sales transactions:
 * DRAFT -> SUBMITTED -> CONFIRMED -> DISPATCHED -> DELIVERED (or CANCELLED).
 */

export type SecondarySaleStatus = 'DRAFT' | 'SUBMITTED' | 'CONFIRMED' | 'DISPATCHED' | 'DELIVERED' | 'CANCELLED';

export interface SecondarySaleProps {
  id: string;
  tenantId: string;
...
export class SecondarySale {
  ...
  private _status: SecondarySaleStatus;
  private _version: number;

  public readonly domainEvents: Array<{ type: string; payload: any }> = [];

  constructor(props: SecondarySaleProps) {
    if (!props.id || !props.tenantId || !props.invoiceNumber || !props.outletId || !props.warehouseId || !props.skuId) {
      throw new Error('SecondarySale must have id, tenantId, invoiceNumber, outletId, warehouseId, and skuId');
    }
    if (props.quantity <= 0) {
  ...
  get status(): SecondarySaleStatus { return this._status; }
  get version(): number { return this._version; }

  static create(props: SecondarySaleProps): SecondarySale {
    const sale = new SecondarySale(props);
    sale.domainEvents.push({
      type: 'distributor.secondary_sale.created',
      payload: {
        id: sale.id,
  ...
  updateStatus(newStatus: SecondarySaleStatus): void {
    if (this._status === 'DELIVERED' || this._status === 'CANCELLED') {
      throw new Error(`Cannot transition from final status ${this._status}`);
    }

    const validTransitions: Record<SecondarySaleStatus, SecondarySaleStatus[]> = {
    ...
    this.domainEvents.push({
      type: 'distributor.secondary_sale.status_updated',
