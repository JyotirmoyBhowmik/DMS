...
export class PurchaseOrderEventConsumer {
  private logger = new StructuredLogger('PurchaseOrderEventConsumer');
  ...
  async handleEvent(eventEnvelope: any): Promise<void> {
    ...
    this.logger.info('Inbound PurchaseOrder event received for ingestion', { eventId, tenantId, type });
    ...
    if (PurchaseOrderEventConsumer.processedEvents.has(eventId)) {
    ...
    try {
      ...
      if (existing.rows.length > 0) {
        PurchaseOrderEventConsumer.processedEvents.add(eventId);
    ...
    this.logger.info('Updating PurchaseOrder projection model', { poId: payload.poId || payload.id, status: payload.status });
    ...
    this.logger.info('PurchaseOrder event processed successfully', { eventId });
  }
}
