...
import { SalesTargetController } from '../../sfa-service/src/presentation/rest/controllers/sales-target.controller.js';
...
beforeEach(() => {
  SalesTargetController.clearStore();
...
describe('Gateway SFA SalesTarget API Integration Tests', () => {
  test('Gateway should route Create SalesTarget successfully (Admin)', async () => {
    ...
    assert.strictEqual((response.body as any).target.id, '00000000-0000-0000-0000-0000000000c1');
    assert.strictEqual((response.body as any).target.status, 'DRAFT');
  });
  ...
  test('Gateway should reject cross-tenant requests (403/404)', async () => {
    ...
    const fetchRequest = {
      method: 'GET',
      path: '/api/v1/sales-targets/00000000-0000-0000-0000-0000000000c3',
      headers: {
        authorization: `Bearer ${adminToken}`,
