...
export class AuditLogEventConsumer {
  private logger = new StructuredLogger('AuditLogEventConsumer');
  ...
  public async handleEvent(event: DomainEvent): Promise<boolean> {
    ...
    try {
      if (event.eventType.startsWith('audit.')) {
        this.logger.info(`Processing audit event: ${event.eventType}`, { eventId: event.eventId });
      ...
      this.logger.error(`Error consuming audit event ${event.eventId}: ${err.message}`);
      this.dlq.push(event);
      return false;
    }
  }
