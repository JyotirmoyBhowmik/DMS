...
export class NotificationEventConsumer {
  private logger = new StructuredLogger('NotificationEventConsumer');
  ...
  public async handleEvent(event: DomainEvent): Promise<boolean> {
    ...
    try {
      if (event.eventType.startsWith('notification.')) {
        this.logger.info(`Processing notification event: ${event.eventType}`, { eventId: event.eventId });
      ...
      this.logger.error(`Error consuming event ${event.eventId}: ${err.message}`);
      this.dlq.push(event);
      return false;
    }
  }
