...
const logger = new StructuredLogger('SchemesBackgroundWorker');

export class SchemesBackgroundWorker {
  ...
  async start(): Promise<void> {
    ...
    logger.info('Starting Schemes background worker components');
    await this.broker.connect();
    
    this.dispatcher.start();
    this.consumer.start();
    logger.info('Schemes background worker started successfully');
  }
  ...
  async stop(): Promise<void> {
    ...
    logger.info('Stopping Schemes background worker components');
    this.dispatcher.stop();
    await this.broker.close();
    await this.db.shutdown();
    logger.info('Schemes background worker stopped successfully');
  }
}
