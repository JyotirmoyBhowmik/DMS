...
export class EventConsumer {
  ...
  // Shared static memory store representing the schemes_processed_events table
  ...
  async consume(envelope: EventEnvelope<any>, handleFn: (event: EventEnvelope<any>) => Promise<void>): Promise<{ status: 'processed' | 'skipped' }> {
    ...
    this.logger.info('Inbound event received for schemes ingestion', { eventId, eventType, tenantId });
    ...
    try {
      ...
      this.logger.info('Event processed successfully and recorded in schemes idempotency store', { eventId, eventType });
      return { status: 'processed' };
    } catch (err: any) {
      this.logger.error('Failed to process inbound event in schemes', { eventId, eventType, error: err.message });
      throw err;
    }
  }
}
