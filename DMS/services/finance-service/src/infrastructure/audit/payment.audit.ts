...
export class PaymentAuditService {
  private logger = new StructuredLogger('PaymentAuditService');
  private static auditTrail: AuditRecord[] = [];

  public static clearAuditTrail(): void {
    PaymentAuditService.auditTrail = [];
  }

  public static getAuditTrail(tenantId: string): AuditRecord[] {
    return PaymentAuditService.auditTrail.filter(r => r.tenantId === tenantId);
  }
  ...
  async recordMutation(record: Omit<AuditRecord, 'id' | 'timestamp'>): Promise<AuditRecord> {
    ...
    const entry: AuditRecord = {
      id: `audit-pay-${Date.now()}-${Math.random().toString(36).substring(2, 7)}`,
    ...
    PaymentAuditService.auditTrail.push(entry);
    this.logger.info(`Payment Audit log recorded: [${entry.action}] for ${entry.entityType}:${entry.entityId} by ${entry.actorId}`);
    return entry;
  }
}
