...
export class ReportScheduleEventConsumer {
  private logger = new StructuredLogger('ReportScheduleEventConsumer');
  ...
  public async handleEvent(event: DomainEvent): Promise<boolean> {
    ...
    try {
      if (event.eventType.startsWith('report.schedule.')) {
        this.logger.info(`Processing report schedule event: ${event.eventType}`, { eventId: event.eventId });
      ...
      this.logger.error(`Error consuming report schedule event ${event.eventId}: ${err.message}`);
      this.dlq.push(event);
      return false;
    }
  }
