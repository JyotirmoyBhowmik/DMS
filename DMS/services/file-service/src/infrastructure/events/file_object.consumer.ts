...
export class FileObjectEventConsumer {
  private logger = new StructuredLogger('FileObjectEventConsumer');
  ...
  public async handleEvent(event: DomainEvent): Promise<boolean> {
    ...
    try {
      if (event.eventType.startsWith('file.')) {
        this.logger.info(`Processing file event: ${event.eventType}`, { eventId: event.eventId });
      ...
      this.logger.error(`Error consuming file event ${event.eventId}: ${err.message}`);
      this.dlq.push(event);
      return false;
    }
  }
