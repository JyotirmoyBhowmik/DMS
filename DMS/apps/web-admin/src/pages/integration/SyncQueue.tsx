...
export const SyncQueue: React.FC<{ role: UserRole }> = () => {
  ...
  const handleRetryTask = (id: string) => {
    setItems((prev) =>
      prev.map((t) => (t.id === id ? { ...t, status: 'SYNCHRONIZED', latency: '24ms (Retried)' } : t))
    );
  };

  const handleReconcileAll = () => {
    setItems((prev) =>
      prev.map((t) => (t.status === 'FAILED' ? { ...t, status: 'SYNCHRONIZED', latency: '40ms (Reconciled)' } : t))
    );
  };
  ...
  return (
    <div style={{ padding: '24px', backgroundColor: tokens.colors.bgApp, minHeight: '100vh', color: tokens.colors.textBody }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: 'bold', color: tokens.colors.textMain, margin: 0 }}>ERP Integration & Reconciliation Ops Board</h1>
          <p style={{ color: tokens.colors.textMuted, margin: '4px 0 0', fontSize: '14px' }}>
            Real-time ERP connector execution logs, field translation errors, and manual ops reconciliation ({items.length} items)
          </p>
        </div>
        {failed > 0 && (
          <button
            onClick={handleReconcileAll}
            style={{ background: '#15803D', color: '#FFF', border: 'none', borderRadius: '6px', padding: '10px 16px', fontWeight: 600, cursor: 'pointer' }}
          >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', marginBottom: '24px' }}>
        <div style={{ backgroundColor: '#FFFFFF', padding: '20px', borderRadius: '8px', border: `1px solid ${tokens.colors.border}`, display: 'flex', alignItems: 'center', gap: '16px' }}>
          <div>
            <div style={{ fontSize: '14px', color: tokens.colors.textMuted, marginBottom: '4px' }}>Total Synced / Reconciled</div>
            <div style={{ fontSize: '24px', fontWeight: 'bold', color: tokens.colors.success }}>{totalSynced}</div>
          </div>
        </div>
        <div style={{ backgroundColor: '#FFFFFF', padding: '20px', borderRadius: '8px', border: `1px solid ${tokens.colors.border}`, display: 'flex', alignItems: 'center', gap: '16px' }}>
          <div>
            <div style={{ fontSize: '14px', color: tokens.colors.textMuted, marginBottom: '4px' }}>Processing Jobs</div>
            <div style={{ fontSize: '24px', fontWeight: 'bold', color: tokens.colors.info }}>{processing}</div>
          </div>
        </div>
        <div style={{ backgroundColor: '#FFFFFF', padding: '20px', borderRadius: '8px', border: `1px solid ${tokens.colors.border}`, display: 'flex', alignItems: 'center', gap: '16px' }}>
          <div>
            <div style={{ fontSize: '14px', color: tokens.colors.textMuted, marginBottom: '4px' }}>Failed / Pending Ops Review</div>
      ...
      {selectedError && (
        <div style={{ background: '#FEF2F2', border: '1px solid #FCA5A5', color: '#991B1B', padding: '16px', borderRadius: '8px', marginBottom: '20px', fontSize: '13px' }}>
          <div style={{ fontWeight: 700, marginBottom: '4px' }}>Ops Inspection Error Traceback:</div>
          <code>{selectedError}</code>
          <button
            onClick={() => setSelectedError(null)}
            style={{ display: 'block', marginTop: '10px', background: '#991B1B', color: '#FFF', border: 'none', padding: '4px 8px', borderRadius: '4px', cursor: 'pointer', fontSize: '12px' }}
          >
            Dismiss Traceback
          </button>
        </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
          <thead>
            <tr style={{ backgroundColor: '#F8FAFC', borderBottom: `1px solid ${tokens.colors.border}` }}>
              ...
              <th style={{ padding: '12px 16px', fontWeight: 600, color: tokens.colors.textBody, fontSize: '13px' }}>Connector Type</th>
              <th style={{ padding: '12px 16px', fontWeight: 600, color: tokens.colors.textBody, fontSize: '13px' }}>Event Payload / Action</th>
              <th style={{ padding: '12px 16px', fontWeight: 600, color: tokens.colors.textBody, fontSize: '13px' }}>Status</th>
              <th style={{ padding: '12px 16px', fontWeight: 600, color: tokens.colors.textBody, fontSize: '13px' }}>Latency</th>
              <th style={{ padding: '12px 16px', fontWeight: 600, color: tokens.colors.textBody, fontSize: '13px' }}>Ops Action</th>
            </tr>
          </thead>
          <tbody>
