...
import { CreateReportDto, ListReportsQueryDto, UpdateReportDto } from '../../../application/dtos/report.dto.js';
...
export class ReportController {
  constructor(
    ...
    private readonly updateUseCase: UpdateReportUseCase,
      ...
      if (!id) {
        return {
          ...
          body: {
            ...
            message: 'Report ID route parameter is required.'
          }
        };
      }
  ...
  public async handleApprove(req: HttpRequest): Promise<HttpResponse> {
    try {
      ...
      const result = await this.updateUseCase.approveReport(principal, id);
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
        body: result
  ...
  public async handleDelete(req: HttpRequest): Promise<HttpResponse> {
    try {
      ...
      const deleted = await this.updateUseCase.deleteReport(principal, id);
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
        body: { success: deleted, message: 'Report deleted successfully' }
      };
    } catch (err: any) {
      return this.mapErrorToResponse(err);
    }
  ...
  private extractPrincipal(req: HttpRequest): Principal {
    ...
    const permissions = (req.headers['x-user-permissions'] || 'report:create,report:read,report:update,report:delete,report:approve').split(',');
