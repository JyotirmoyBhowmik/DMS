...
import { CreatePriceSlabSchema, UpdatePriceSlabSchema, QueryPriceSlabSchema } from '@dms/pkg-validation';
...
export class PriceSlabController {
  private db = new PostgresDatabaseClient(config.db, new PgDriver());
  private repo = new PriceSlabPgRepository(this.db);
  private createUseCase = new CreatePriceSlabUseCase(this.repo);
  private getUseCase = new GetPriceSlabUseCase(this.repo);
  private updateUseCase = new UpdatePriceSlabUseCase(this.repo);
  private listUseCase = new ListPriceSlabsUseCase(this.repo);
  private logger = new StructuredLogger('PriceSlabController');
  ...
  async handleCreate(body: any, headers: Record<string, string | undefined>) {
    ...
    this.logger.info('Received HTTP POST price slab request', { tenantId, idempotencyKey });
    const principal = this.buildPrincipal(headers);

    try {
      const parsed = CreatePriceSlabSchema.parse(body);
      const slab = await this.createUseCase.execute(principal, parsed, idempotencyKey);

      return {
        statusCode: 201,
        body: {
          success: true,
          priceSlab: slab.toJSON(),
        },
      };
    } catch (err: any) {
      this.logger.warn('Validation or execution failed for create price slab', { error: err.message });
  ...
  async handleGet(id: string, headers: Record<string, string | undefined>) {
    const principal = this.buildPrincipal(headers);
    try {
      const slab = await this.getUseCase.execute(principal, id);
      if (!slab) {
        return { statusCode: 404, body: { success: false, error: 'PriceSlab record not found' } };
      }
      return { statusCode: 200, body: { success: true, priceSlab: slab.toJSON() } };
    } catch (err: any) {
  ...
  async handleUpdate(id: string, body: any, headers: Record<string, string | undefined>) {
    const principal = this.buildPrincipal(headers);
    try {
      const parsed = UpdatePriceSlabSchema.parse(body);
      const updated = await this.updateUseCase.execute(principal, id, parsed);
      return { statusCode: 200, body: { success: true, priceSlab: updated.toJSON() } };
    } catch (err: any) {
      const isForbidden = err.message.includes('Forbidden');
  ...
  async handleList(query: any, headers: Record<string, string | undefined>) {
    const principal = this.buildPrincipal(headers);
    try {
      const parsed = QueryPriceSlabSchema.parse(query);
