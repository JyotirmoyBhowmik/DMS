import { CreateProductCategoryUseCase } from '../../../application/usecases/product_category/create-product-category.usecase.js';
import { GetProductCategoryUseCase } from '../../../application/usecases/product_category/get-product-category.usecase.js';
import { UpdateProductCategoryUseCase } from '../../../application/usecases/product_category/update-product-category.usecase.js';
import { ListProductCategoriesUseCase } from '../../../application/usecases/product_category/list-product-categories.usecase.js';
import { ProductCategoryPgRepository } from '../../../infrastructure/database/repositories/product_category.pg-repository.js';
import { CreateProductCategorySchema, UpdateProductCategorySchema, QueryProductCategorySchema } from '@dms/pkg-validation';
import { Principal } from '@dms/pkg-rbac';
import { StructuredLogger } from '@dms/pkg-logger';
import { PostgresDatabaseClient, PgDriver } from '@dms/pkg-database';
import { loadConfigSync } from '@dms/pkg-config';

const config = loadConfigSync();

export class ProductCategoryController {
  private db = new PostgresDatabaseClient(config.db, new PgDriver());
  private repo = new ProductCategoryPgRepository(this.db);
  private createUseCase = new CreateProductCategoryUseCase(this.repo);
  private getUseCase = new GetProductCategoryUseCase(this.repo);
  private updateUseCase = new UpdateProductCategoryUseCase(this.repo);
  private listUseCase = new ListProductCategoriesUseCase(this.repo);
  private logger = new StructuredLogger('ProductCategoryController');

  static clearStore(): void {
    ProductCategoryPgRepository.clearStore();
  }

  private buildPrincipal(headers: Record<string, string | undefined>): Principal {
    const tenantId = headers['x-tenant-id'] || '00000000-0000-0000-0000-000000000001';
    const roles = headers['x-user-roles'] ? (headers['x-user-roles'] as string).split(',') : ['admin'];
    return {
      id: headers['x-user-id'] || 'mock-user-uuid',
      tenantId,
      roles,
    };
  }

  async handleCreate(body: any, headers: Record<string, string | undefined>) {
    const tenantId = headers['x-tenant-id'] || '00000000-0000-0000-0000-000000000001';
    const idempotencyKey = headers['x-idempotency-key'];
    this.logger.info('Received HTTP POST product category request', { tenantId, idempotencyKey });
    const principal = this.buildPrincipal(headers);

    try {
      const parsed = CreateProductCategorySchema.parse(body);
      const category = await this.createUseCase.execute(principal, parsed, idempotencyKey);

      return {
        statusCode: 201,
        body: {
          success: true,
          category: category.toJSON(),
        },
      };
    } catch (err: any) {
      this.logger.warn('Validation or execution failed for create category', { error: err.message });
      const isForbidden = err.message.includes('Forbidden') || err.message.includes('permission');
      const isConflict = err.message.includes('409 Conflict') || err.message.includes('already exists');
      return {
        statusCode: isForbidden ? 403 : (isConflict ? 409 : 400),
        body: { success: false, error: err.message },
      };
    }
  }

  async handleGet(id: string, headers: Record<string, string | undefined>) {
    const principal = this.buildPrincipal(headers);
    try {
      const category = await this.getUseCase.execute(principal, id);
      if (!category) {
        return { statusCode: 404, body: { success: false, error: 'ProductCategory not found' } };
      }
      return { statusCode: 200, body: { success: true, category: category.toJSON() } };
    } catch (err: any) {
      const isForbidden = err.message.includes('Forbidden');
      return { statusCode: isForbidden ? 403 : 400, body: { success: false, error: err.message } };
    }
  }

  async handleUpdate(id: string, body: any, headers: Record<string, string | undefined>) {
    const principal = this.buildPrincipal(headers);
    try {
      const parsed = UpdateProductCategorySchema.parse(body);
      const updated = await this.updateUseCase.execute(principal, id, parsed);
      return { statusCode: 200, body: { success: true, category: updated.toJSON() } };
    } catch (err: any) {
      const isForbidden = err.message.includes('Forbidden');
      const isConflict = err.message.includes('Conflict') || err.message.includes('version');
      const isNotFound = err.message.includes('not found');
      let statusCode = 400;
      if (isForbidden) statusCode = 403;
      else if (isConflict) statusCode = 409;
      else if (isNotFound) statusCode = 404;
      return { statusCode, body: { success: false, error: err.message } };
    }
  }

  async handleList(query: any, headers: Record<string, string | undefined>) {
    const principal = this.buildPrincipal(headers);
    try {
      const parsed = QueryProductCategorySchema.parse(query);
      const result = await this.listUseCase.execute(principal, parsed);
      return {
        statusCode: 200,
        body: {
          success: true,
          ...result,
          data: result.data.map(c => c.toJSON()),
        },
      };
    } catch (err: any) {
      const isForbidden = err.message.includes('Forbidden');
      return { statusCode: isForbidden ? 403 : 400, body: { success: false, error: err.message } };
    }
  }
}
