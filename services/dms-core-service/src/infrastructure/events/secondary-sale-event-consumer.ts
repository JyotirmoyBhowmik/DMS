...
export class SecondarySaleEventConsumer {
  private logger = new StructuredLogger('SecondarySaleEventConsumer');
  ...
  async handleEvent(eventEnvelope: any): Promise<void> {
    ...
    this.logger.info('Inbound SecondarySale event received for ingestion', { eventId, tenantId, type });
    ...
    if (SecondarySaleEventConsumer.processedEvents.has(eventId)) {
    ...
    try {
      ...
      if (existing.rows.length > 0) {
        SecondarySaleEventConsumer.processedEvents.add(eventId);
    ...
    this.logger.info('Updating SecondarySale projection model', { saleId: payload.saleId || payload.id, status: payload.status });
    ...
    this.logger.info('SecondarySale event processed successfully', { eventId });
  }
}
