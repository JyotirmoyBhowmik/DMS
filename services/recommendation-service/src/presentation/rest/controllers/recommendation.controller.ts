...
import { CreateRecommendationDto, ListRecommendationsQueryDto, UpdateRecommendationDto } from '../../../application/dtos/recommendation.dto.js';
...
export class RecommendationController {
  constructor(
    ...
    private readonly getUseCase: GetRecommendationUseCase,
    private readonly updateUseCase: UpdateRecommendationUseCase,
    private readonly listUseCase: ListRecommendationsUseCase
  ) {}
      ...
      if (!id) {
        return {
          ...
          body: {
            ...
            message: 'Recommendation ID route parameter is required.'
          }
        };
      }
  ...
  public async handleApprove(req: HttpRequest): Promise<HttpResponse> {
    try {
      ...
      const result = await this.updateUseCase.approveRecommendation(principal, id, expectedVersion);
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
        body: result
  ...
  public async handleDelete(req: HttpRequest): Promise<HttpResponse> {
    try {
      ...
      const deleted = await this.updateUseCase.deleteRecommendation(principal, id);
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
        body: { success: deleted, message: 'Recommendation deleted successfully' }
      };
    } catch (err: any) {
      return this.mapErrorToResponse(err);
    }
  ...
  public async handleList(req: HttpRequest): Promise<HttpResponse> {
    try {
      const principal = this.extractPrincipal(req);
      const query: ListRecommendationsQueryDto = {
  ...
  private extractPrincipal(req: HttpRequest): Principal {
    ...
    const permissions = (req.headers['x-user-permissions'] || 'recommendation:create,recommendation:read,recommendation:update,recommendation:delete').split(',');
