...
export class ForecastModelEventConsumer {
  private logger = new StructuredLogger('ForecastModelEventConsumer');
  ...
  public async handleEvent(event: DomainEvent): Promise<boolean> {
    ...
    try {
      if (event.eventType.startsWith('forecast.model.')) {
        this.logger.info(`Processing forecast model event: ${event.eventType}`, { eventId: event.eventId });
      ...
      this.logger.error(`Error consuming forecast model event ${event.eventId}: ${err.message}`);
      this.dlq.push(event);
      return false;
    }
  }
