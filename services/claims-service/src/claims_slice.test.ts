...
describe('Claims Module & E2E Integration Tests', () => {
  ...
  before(async () => {
    ...
    try {
      ...
      console.log('Skipping live Claims Database Integration Tests because postgres is not reachable.');
      return;
    }
  ...
  beforeEach(async () => {
    ...
    await db.query('TRUNCATE TABLE claims, claims_audit_history, claims_outbox, claim_settlement_transactions RESTART IDENTITY CASCADE');
  });
  ...
  test('Domain: ClaimAggregate validates invariants, state machine transitions, and over-claim checks', () => {
    ...
    // Invariant: settledAmount must be non-negative
    assert.throws(() => {
      const entity = new ClaimEntity({
        tenantId: tenantA,
        distributorId,
      ...
      new ClaimAggregate(entity).validateInvariants();
    }, /settledAmount must be non-negative/);
    ...
    // Cannot approve/settle in raised
  ...
  test('E2E: Full lifecycle via API Gateway, concurrency guard, and audit history check', async () => {
    ...
    assert.strictEqual(auditRows.rows.length, 4); // raised, validated, approved, settle
    ...
    const outboxRows = await db.query<any>(
      `SELECT * FROM claims_outbox WHERE aggregate_id = $1 ORDER BY created_at ASC`,
      [claimId],
      tenantA
    );
