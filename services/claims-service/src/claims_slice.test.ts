...
describe('Claims Module & E2E Integration Tests', () => {
  ...
  test('E2E: Full lifecycle via API Gateway, concurrency guard, and audit history check', async () => {
    ...
    assert.strictEqual((validateResult.body as any).claim.status, 'UNDER_REVIEW');
    ...
    assert.strictEqual((approveResult.body as any).claim.status, 'APPROVED');
    ...
    assert.strictEqual(((settleResult.body as any).claim).status, 'SETTLED');
