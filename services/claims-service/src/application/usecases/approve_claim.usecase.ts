...
export class ApproveClaimUseCase {
  private logger = new StructuredLogger('ApproveClaimUseCase');
  ...
  async execute(tenantId: string, claimId: string, actorId: string): Promise<{ status: string }> {
    this.logger.info('Approving claim', { claimId });

    if (!this.db) {
      throw new Error('Database client required for approve use case');
    }
