...
ALTER TABLE IF EXISTS secondary_sales
  DROP CONSTRAINT IF EXISTS chk_secondary_sales_positive_quantity;
ALTER TABLE IF EXISTS secondary_sales
  ADD CONSTRAINT chk_secondary_sales_positive_quantity CHECK (quantity > 0);

ALTER TABLE IF EXISTS secondary_sales
  DROP CONSTRAINT IF EXISTS chk_secondary_sales_non_negative_amounts;
ALTER TABLE IF EXISTS secondary_sales
  ADD CONSTRAINT chk_secondary_sales_non_negative_amounts CHECK (unit_price_cents >= 0 AND total_amount_cents >= 0);

-- Indexes for performance and common filters
CREATE INDEX IF NOT EXISTS idx_secondary_sales_tenant_status ON secondary_sales (tenant_id, status);
CREATE INDEX IF NOT EXISTS idx_secondary_sales_outlet_wh ON secondary_sales (tenant_id, outlet_id, warehouse_id);

-- Row-Level Security Policy
ALTER TABLE secondary_sales ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS secondary_sales_tenant_isolation_policy ON secondary_sales;
CREATE POLICY secondary_sales_tenant_isolation_policy ON secondary_sales
  FOR ALL
