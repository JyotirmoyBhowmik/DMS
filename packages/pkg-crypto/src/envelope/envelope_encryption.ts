...
export class EnvelopeEncryptionService {
  private static _platformKek: Buffer | null = null;

  private static get platformKek(): Buffer {
    if (!this._platformKek) {
      const secret = process.env.PLATFORM_KEK_SECRET || process.env.LEGACY_PLATFORM_KEK_SECRET;
      if (!secret) {
        throw new Error(
          'Missing environment variable: PLATFORM_KEK_SECRET or LEGACY_PLATFORM_KEK_SECRET is required',
        );
      }
      this._platformKek = crypto.scryptSync(secret, 'platform-kek-salt', 32);
    }
    return this._platformKek;
  }
